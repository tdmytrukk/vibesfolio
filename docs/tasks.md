# Vibesfolio — Implementation Tasks

> Source of truth for implementation order. Updated as work progresses.
> Phase 1–4 are complete. This tracks Phase 5 (Polish & Growth) and beyond.

## Status Key
- ⬜ Not started
- 🔨 In progress
- ✅ Done
- ⏸️ Blocked / Deferred

---

## Phase 5A — Critical Polish (Mobile & UX Fixes)

| # | Task | Status | Notes |
|---|------|--------|-------|
| 5A.1 | Fix mobile modal Save button visibility (keyboard overlap) | ⬜ | Sticky footer in modals so Save stays visible above keyboard |
| 5A.2 | Add success toasts for all CRUD operations | ⬜ | Consistent feedback: "Idea saved", "Resource added", etc. |
| 5A.3 | Wire `generate-tags` edge function into add flows | ⬜ | Auto-suggest tags when adding ideas, prompts, resources |
| 5A.4 | Auto-populate resource title/description from metadata | ⬜ | `fetch-url-metadata` already returns ogTitle + ogDescription; wire into AddResourceModal |
| 5A.5 | Add `updated_at` triggers for ideas, project_tasks, project_missions | ⬜ | DB migration — match the builds trigger pattern |
| 5A.6 | Empty state improvements across all pages | ⬜ | Contextual CTAs: "Save your first prompt", "Start a project", etc. |

## Phase 5B — Search & Discovery

| # | Task | Status | Notes |
|---|------|--------|-------|
| 5B.1 | Global search component in nav bar | ⬜ | Search across ideas, prompts, resources, builds by title/tags |
| 5B.2 | Search results page or dropdown with categorized results | ⬜ | Group by type: Ideas (3), Prompts (5), Resources (2), Projects (1) |
| 5B.3 | Add full-text search indexes to DB tables | ⬜ | DB migration for `tsvector` columns or use Supabase `textSearch` |

## Phase 5C — Onboarding

| # | Task | Status | Notes |
|---|------|--------|-------|
| 5C.1 | New user detection (first login with empty data) | ⬜ | Check if user has 0 builds + 0 ideas + 0 resources |
| 5C.2 | Welcome screen / guided tour | ⬜ | 3–4 step intro: "Capture ideas → Save resources → Track builds → Reflect" |
| 5C.3 | Change default landing for new users | ⬜ | New users → onboarding. Returning users → `/log` (Projects) |
| 5C.4 | Seed sample data option | ⬜ | "Start with example project" to show cockpit features |

## Phase 5D — Analytics & Insights

| # | Task | Status | Notes |
|---|------|--------|-------|
| 5D.1 | Personal stats dashboard | ⬜ | Ideas captured, resources saved, prompts created, builds shipped, debriefs completed |
| 5D.2 | Build activity timeline | ⬜ | Visual timeline of shipping log + debriefs across all projects |
| 5D.3 | Streak / consistency tracking | ⬜ | "You've built 5 days in a row" — debrief completion streak |
| 5D.4 | Ideas → Builds conversion tracking | ⬜ | Requires `source_idea_id` FK on builds table (DB migration) |

## Phase 5E — AI Enhancements

| # | Task | Status | Notes |
|---|------|--------|-------|
| 5E.1 | AI tag suggestions on save (ideas, prompts, resources) | ⬜ | Uses existing `generate-tags` edge function + Lovable AI gateway |
| 5E.2 | AI-powered prompt refinement | ⬜ | "Improve this prompt" button → AI rewrites with better structure |
| 5E.3 | Smart next-step suggestions in Cockpit | ⬜ | Based on current tasks + recent debrief, suggest next mission |
| 5E.4 | Auto-categorize resources | ⬜ | AI determines tool/article/video/other from URL metadata |

## Phase 6 — Future Features

| # | Task | Status | Notes |
|---|------|--------|-------|
| 6.1 | Public builder portfolio pages | ⬜ | `/builders/:userId` — public page showing shared artifacts + build count |
| 6.2 | Curated prompt packs | ⬜ | Collections of prompts bundled as "packs" with descriptions |
| 6.3 | Export/import data | ⬜ | JSON export of all user data; import from JSON |
| 6.4 | GitHub integration | ⬜ | Link builds to repos; auto-log commits as shipping entries |
| 6.5 | Pro tier with gating | ⬜ | Stripe integration; limit free tier artifacts; unlock AI features |
| 6.6 | Prompt effectiveness tracking | ⬜ | Rate prompts after use; community sorting by effectiveness |
| 6.7 | Dark mode toggle in UI | ⬜ | CSS tokens exist; add toggle in profile/nav |

---

## Suggested Implementation Order

**Start with 5A (Critical Polish)** — fixes real bugs and wires up existing-but-unused features. High impact, low effort.

Then: **5B (Search)** → **5C (Onboarding)** → **5E (AI)** → **5D (Analytics)** → **Phase 6**

Search and onboarding are retention drivers. AI features add delight but aren't blockers. Analytics can be built once there's enough usage data to make them meaningful.

---

*Last updated: 2026-02-27*
