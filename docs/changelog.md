...
- **2026-03-01**: Dark mode support — wired `next-themes` ThemeProvider, added theme picker to onboarding WelcomeTour (3 steps now), added Appearance toggle to ProfilePage, dark gradient + texture overlay, `.nav-glass`/`.mobile-nav-glass` CSS classes replacing inline styles (`src/App.tsx`, `src/index.css`, `src/components/WelcomeTour.tsx`, `src/pages/ProfilePage.tsx`, `src/components/AppShell.tsx`)
